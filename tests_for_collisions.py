...
class Testing(unittest.TestCase):
  ...
  def test_find_intersection_1(self):
    t = math.sqrt(2)
    spheres = [data.Sphere(data.Point(1,1,0), t), data.Sphere(data.Point(2,2,0), t), data.Sphere(data.Point(3,3,0), t)]
    ray = data.Ray(data.Point(0,0,0), data.Vector(1,1,0))
    result = [(spheres[0], data.Point(0, 0, 0)), (spheres[1], data.Point(1,1, 0)), (spheres[2], data.Point(2,2, 0))]
    self.assertEqual(collisions.find_intersection_points(spheres, ray)==result, True)
    pass
  def test_find_intersection_2(self):
    spheres = [data.Sphere(data.Point(1,1,1), 1), data.Sphere(data.Point(2,2,2), 1), data.Sphere(data.Point(3,3,3), 1)]
    ray = data.Ray(data.Point(0,0,0), data.Vector(1,0,0))
    result = []
    self.assertEqual(collisions.find_intersection_points(spheres, ray)==result, True)
    pass
